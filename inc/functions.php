...
function sessionExistsWithMotions ($session_id)
{
    ...
    // if ($result) {
	// 	if (pg_num_rows ($result) > 0 && strlen($row[2]) > 0) {

	// 		$bla = pg_num_rows($result);
	// 		var_dump($bla);
	// 		$blaa = strlen($row[2]) > 0;
	// 		var_dump($blaa);

	// 		$blaaa = pg_fetch_assoc($result);
	// 		var_dump($blaaa);
    ...
    // if ($result) {
	// 	if ((pg_num_rows ($result) > 0) && (strlen($mRow[2]) > 0)) {
	// 		return pg_fetch_assoc($result);
	// 	}
	// }
	// return false;
		...
		if ($date < new DateTime('NOW')) {
			...
			if (PARSE_VOTES) {
				...
				if (!empty($matchesp[1]) && (int)$matchesp[1] > 0) {
					// this was a hack
					// var_dump($session_nouid);
					// if ($exists2 = sessionExists ($session_nouid)) {
					// 	var_dump($exists2);
					// 	$tmp['id'] = $exists2['id']; // Set that session exists
					// 	var_dump($tmp);
					// 	// $tmp['review_ext'] = true;
					// }
