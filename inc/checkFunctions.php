...
function motionExists($session_id, $organization_id, $date, $name, $uid)
{
...
function checkIfSpeechInsertIsUnnecesaryParsed($speech_date, $speech)
{
    ...
    foreach ($speech['talks'] as $talk) {
        ...
        $hash .= md5($talk['id'] . $talk['vsebina'] . $order . $speech_date . getPersonOrganization($talk['id'], $speech_date));
    }
